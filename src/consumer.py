from .schema import Schema, SchemaState
...
class SchemaConsumer:
    ...
    def __init__(self, schema: Schema) -> None:
        """Initialize the consumer with a schema to update."""

        self.schema = schema

    def consume(self, token_id: int) -> None:
        """Consume a token and update the schema state accordingly."""
    ...
    def consume_function_token(self, token_id: int) -> None:
        """Consume a token for the function name
        and update the schema state."""
    ...
    def consume_parameter_token(self, token_id: int) -> None:
        """Consume a token for the parameter name
        and update the schema state."""
    ...
    def consume_parameter_end(self, token_id: int) -> None:
        """Consume a token that indicates the end of a parameter value
        and update the schema state."""
    ...
    def consume_value_token(self, token_id: int) -> None:
        """Consume a token for the parameter value
        and update the schema state."""
    ...
    def advance_state(self) -> None:
        ...
        transitions = {
            ...
            SchemaState.EXPECT_PARAMETERS_KEY: (
                SchemaState.EXPECT_PARAMETERS_COLON
            ),
            SchemaState.EXPECT_PARAMETERS_COLON: (
                SchemaState.EXPECT_PARAMETERS_OPEN
            ),
            SchemaState.EXPECT_PARAMETERS_OPEN: (
                SchemaState.EXPECT_PARAMETER_NAME
            ),
            SchemaState.EXPECT_PARAMETER_NAME: (
                SchemaState.EXPECT_PARAMETER_COLON
            ),
            SchemaState.EXPECT_PARAMETER_COLON: (
                SchemaState.EXPECT_PARAMETER_VALUE
            ),
            SchemaState.EXPECT_PARAMETER_VALUE: (
                SchemaState.EXPECT_PARAMETER_OR_END
            ),
